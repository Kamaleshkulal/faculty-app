...
const EditFacultyPopup = ({ isOpen, onClose, facultyDetails, data }) => {
    ...
    const removeDropdown = async (index) => {
        if (dropdowns.length > 1) {
            const subjectIdToDelete = dropdowns[index].subject;

            try {
                const response = await fetch(`http://localhost:8080/v1/subjects/${subjectIdToDelete}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        // Add any necessary headers (authorization, etc.) here
                    },
                });

                if (!response.ok) {
                    throw new Error(`Failed to delete subject with ID ${subjectIdToDelete}`);
                }

                console.log(`Subject with ID ${subjectIdToDelete} deleted successfully`);

                // Update dropdowns state after successful deletion
    ...
    const handleSubmit = async (e) => {
        ...
        const updatedFacultyDetails = {
            ...
            facultyFatherName: facultyDetails.facultyFatherName,
